...
USDVIEW_VERSION=${REZ_BUILD_PROJECT_VERSION}
...
echo -e "[CONFIGURE][ARGS] USDVIEW VERSION: ${USDVIEW_VERSION}"

# We check if the arguments variables we need are correctly set.
# If not, we abort the process.
if [[ -z ${EXTRACT_PATH} || -z ${BUILD_PATH} || -z ${INSTALL_PATH} || -z ${USDVIEW_VERSION} ]]; then
    echo -e "\n"
...
# We run the configuration script of usdview.
echo -e "\n"
echo -e "[CONFIGURE] Running the configuration script from usdview-${USDVIEW_VERSION}..."
echo -e "\n"
...
echo -e "\n"
echo -e "[CONFIGURE] Finished configuring usdview-${USDVIEW_VERSION}!"
echo -e "\n"
